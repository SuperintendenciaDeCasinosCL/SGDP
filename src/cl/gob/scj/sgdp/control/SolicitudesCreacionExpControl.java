...
import cl.gob.scj.sgdp.dto.ArchivosSolCreaExpDTOJsonObject;
...
public class SolicitudesCreacionExpControl {
	...
	public String solicitudesCreacionExp(Model model, HttpServletRequest request) {
		...
		try {
			...
			solicitudesCreacionExpDTO = solicitudesCreacionExpService.getSolicitudesCreaExpSolicitadasPorOAsignadasA(solicitudCreacionExpDTO, usuario);
	...
	//MIG
	@RequestMapping(value = "/getArchivosCrearExp/{idSolicitudCreacionExp}", method = RequestMethod.GET, produces = "application/json; charset=utf-8")
	public @ResponseBody String getArchivosCrearExp(@PathVariable("idSolicitudCreacionExp") int idSolicitudCreacionExp, HttpServletRequest  request) throws IOException {
		log.info("Inicio getArchivosCrearExp");
		log.info("Inicio idSolicitudCreacionExp" + idSolicitudCreacionExp);
		Integer pagina = 0;
		pagina = Integer.valueOf(request.getParameter("iDisplayStart"));
		int tamanoPagina = Integer.valueOf(request.getParameter("iDisplayLength"));
		ArchivosSolCreaExpDTO archivosSolCreaExpDTO = new ArchivosSolCreaExpDTO();
		archivosSolCreaExpDTO.setIdSolicitudCreacionExp(idSolicitudCreacionExp);
		archivosSolCreaExpDTO.setTamanoPagina(tamanoPagina);
		archivosSolCreaExpDTO.setPagina(pagina);

		Usuario usuario = (Usuario)request.getSession().getAttribute("usuario");
		List<ArchivosSolCreaExpDTO> listaObteneDoc = null;

		try {
			listaObteneDoc = solicitudesCreacionExpService.getArchivosCrearExp(archivosSolCreaExpDTO, usuario);
			log.info("listaObteneDoc" + listaObteneDoc.size());
			int totalRegistros = listaObteneDoc.size();
			ArchivosSolCreaExpDTOJsonObject arcFinJO = new ArchivosSolCreaExpDTOJsonObject();
			arcFinJO.setAaData(listaObteneDoc);
			arcFinJO.setiTotalDisplayRecords(totalRegistros);
			arcFinJO.setiTotalRecords(totalRegistros);
			String responseJson;
			ObjectMapper mapper = SingleObjectFactory.getMapper();
			responseJson = mapper.writeValueAsString(arcFinJO);
			return responseJson;
		} catch (Exception e) {
			StringWriter sw = new StringWriter();
			e.printStackTrace(new PrintWriter(sw));
