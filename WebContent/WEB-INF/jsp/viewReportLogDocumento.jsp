			    ...
			    <div class="col-sm-10">	
			    	...
			    	<c:choose>
						<c:when test = "${usuario.fueraDeOficina eq true}">
							...
							<c:set var="pageHeaderMessage" value="Visualizador Log Documentos (Fuera de Oficina)" />
						</c:when>
						<c:otherwise>
							<c:set var="headerBackColor" value="#41CAC0" />
							<c:set var="pageHeaderMessage" value="Visualizador Log Documentos " />							
						</c:otherwise>						
					</c:choose> 
					...
					<div id="divBackHeaderMantenedorParametros" class="row div-area-trabajo-cab" style="background-color: ${headerBackColor}; color: #fff;">
		      		...
		      		<hr>
   					...
   					<div id="bodyTable" class="col-sm-12">
   						<c:import url="/WEB-INF/jsp/div/bodyReportLogDocumento.jsp"></c:import>
   						<c:import url="/WEB-INF/jsp/modals/viewLogDocument.jsp"></c:import>
				   </div>  
